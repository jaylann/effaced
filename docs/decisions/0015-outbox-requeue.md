...
## Amendment (2026-06-12, in #90): requeue is erase-only

The "mechanically operation-agnostic" framing above (and the planned `RECTIFICATION_REQUEUED`) collided with ADR 0013 during implementation and was wrong. ADR 0013 clears a rectify entry's `payload` — the corrections, which are real PII — on **every** terminal transition, abandonment included. An abandoned rectify entry therefore has no corrections left to re-apply. Requeuing it to `PENDING` would let the runner re-execute `rectify_subject(ref, ())`, apply nothing, mark it `SUCCEEDED`, and fire `RECTIFICATION_COMPLETED` — a silent no-op rectification, an Art. 16 correctness hole.

So `requeue` is **erase-only**. An `ABANDONED` rectify entry among the ids raises `ConfigurationError` naming it, before any audit append or status change (validation-first, nothing flips, no event). The remediation for an abandoned rectification is to **re-issue it through the `Rectifier`**, which enqueues a fresh entry carrying the corrections again — the corrections live with the operator's intent, not in a terminal outbox row. Consequently `RECTIFICATION_REQUEUED` is **not** minted: only `ERASURE_REQUEUED` exists, and removing the speculative member before it ever shipped avoids a MAJOR enum removal later. The forward-reference in ADR 0013's consequences is superseded by this note.
