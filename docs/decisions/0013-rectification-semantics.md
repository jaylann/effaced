...
- Resolvers that implement `rectify_subject` inherit the conformance suite's new idempotency obligation: rectify twice, expect `already_consistent=True` the second time (ADR 0018's suite grows a rectification section when this ships).
- ADR 0015's requeue cannot apply to rectify entries *because* of the payload-clearing rule above: an abandoned rectify entry has no corrections left to re-apply, so requeue is erase-only and an abandoned rectification is remediated by re-issuing it through the `Rectifier` (which re-enqueues the corrections). See ADR 0015's "requeue is erase-only" amendment; no `RECTIFICATION_REQUEUED` event exists.
