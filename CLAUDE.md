...
- A3 (export memory safety): the streaming paths are pure refactors that the materializing entry points now delegate *through*, so equivalence is structural, not a parallel reimplementation — `Exporter.export_subject` keeps building the bundle but `_local_records` is now `tuple(_iter_local_records(...))`, `collect_object_records` is `tuple(iter_object_records(...))`. The acceptance gate (`test_exporter_streaming.py`, `test_export_collection.py`) still asserts `streamed == materialized` to catch a future fork. The batched ANONYMIZE in `erasure_executor._anonymize` does NOT use a streaming cursor (interleaving a `yield_per` read with `UPDATE` statements on the same table/connection is driver-fragile). It pages by a **keyset cursor** (`order_by(pk).where(pk > last).limit(_BATCH)`), NOT `offset(done)` — an OFFSET walk is unsafe whenever an anonymized cell can shift which rows match or reorder them, and the original "safe because the anonymized columns are never the ordering key" was an *unenforced* assumption: `_table_steps` can legally emit a PK column as an ANONYMIZE target, and a `String`/`Uuid` PK draws a fresh unique surrogate, mutating the ordering key mid-walk and skipping rows (PII survives — reviewer-caught HIGH on #162). Keyset paging advances by the key value, so it is immune to count drift, but only while the cursor column is itself stable; therefore when the PK is **composite** (`len(key) != 1`) OR **is itself an anonymize target** (`key[0].name in columns`) the executor captures every matching key in ONE up-front `select` and rewrites row by row (correct, at the cost of the resident key set — the old all-keys-first behaviour, scoped to exactly the cases that need it). Per-row fresh surrogates (ADR 0007) are unchanged, so the erased result is byte-identical to the materializing path; tests force `_BATCH=3/4` via monkeypatch to span batch boundaries, and `test_anonymizing_a_string_primary_key_skips_no_rows_across_batches` covers the anonymized-PK path specifically.
...
- Optional resolver capabilities follow one settled shape: a `@runtime_checkable class XResolver(Resolver, Protocol)` adding the new async method, a frozen `extra="forbid"` result model mirroring `ResolverErasure`, a saga-runner `isinstance` narrow at the call site, and silent skip for non-implementers (no event, no error). `VerifyingResolver`/`ResolverVerification` (ADR 0027) join `Rectifying`/`Attesting`/`RetentionOnly`. The conformance suite grows a `make_*_resolver()` hook (returns `None` ⇒ skip) + an `isinstance`-gated `_x_resolver()` helper + tests, and `InMemoryResolver` grows the method so the self-test exercises it.
- The saga runner's settlement (`_settle`/`_succeed`) is sync (runs after `asyncio.gather`), so any new *await* a capability needs must happen inside `_execute` (the async phase). To thread an extra async result through to the sync bookkeeping, return a small internal frozen carrier (`saga/verified_erasure.py::VerifiedErasure`) from `_execute` and unwrap it at the top of `_settle` — keeps the existing dispatch unions otherwise untouched.
- A post-success assurance (verify_absent) must be isolated like the abandonment hook: its raise is swallowed (`except Exception: return None`) so it can never undo or retry an already-`SUCCEEDED` erase. Verification is additional trail, never a gate — a negative/failed verdict is audited (`ERASURE_EXTERNAL_VERIFICATION_FAILED`) but the entry stays SUCCEEDED and completion still fires.
