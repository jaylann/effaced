...
- `EffacedStack.from_base` mounts the owned tables on the passed Base's metadata at call time: a fixture that ran `create_all` *before* `from_base` must run it again (idempotent) or the `effaced_*` tables don't exist in SQLite.
