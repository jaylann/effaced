...
## Resolvers & saga
- `Resolver` and `AuditSink` protocols are public API with the strictest stability promise: extend **additively only** (optional methods with default impls). Never change existing signatures. Capability growth is a separate `@runtime_checkable` protocol + `isinstance` narrowing at call sites, leaving the base protocol literally untouched — `RectifyingResolver` (extends `Resolver`) and `ReplaySource` (`replay/source.py`, standalone — deliberately not an `AuditSink` method; ADR 0018) are the precedents.
