...
## Owned-table schema (ADR 0021)
- effaced never ships Alembic migration scripts and never touches `alembic_version` — callers author migrations against their own `target_metadata`, onto which `bind_tables()` mounts the owned tables.
- Owned-table DDL evolves **additively only** in MINOR releases. A new column on an owned table must be nullable **or** carry a `server_default` so the caller's autogenerated additive ALTER backfills populated tables in place (`effaced_outbox.operation` precedent, ADR 0013). Drop/rename/retype is MAJOR.
- Index names stay explicit and stable; a second autogenerate after applying must yield zero diffs — `test_alembic_autogenerate_pg.py` pins it, and a new server default also extends `test_no_server_defaults_except_the_outbox_operation_migration_aid` in the same PR.
- Every owned-table DDL change ships a changelog note: "re-run `alembic revision --autogenerate` after upgrading". `MANIFEST_SCHEMA_VERSION` is orthogonal — manifests are serialized JSON, never DB rows; a manifest bump alone never requires a DB migration.
...
## Resolvers & saga
- `Resolver` and `AuditSink` protocols are public API with the strictest stability promise: extend **additively only**. Capability growth is a separate `@runtime_checkable` protocol + `isinstance` narrowing at call sites, leaving the base protocol untouched: use `class XResolver(Resolver, Protocol)` for resolver capabilities (`RectifyingResolver`, `RetentionOnlyResolver`) and standalone protocols for non-resolver capabilities (`ReplaySource`, deliberately not an `AuditSink` method; ADR 0023). Never change existing signatures.
