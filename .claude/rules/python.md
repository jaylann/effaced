...
- Core (`effaced/*` except `adapters/`) must not import SQLAlchemy or any storage library (semgrep-gated); storage-specific code lives in `effaced/adapters/<stack>/`. Core can still issue SQL by building statements off the bound `Table` handle (`table.select()`, `.update()`, `c.col.in_(...)`, `.with_for_update(skip_locked=True)`) — no runtime import needed. `Select.with_only_columns()` recalculates the FROM list from the new columns plus later `.where()` criteria — selecting a hop-chain alias's column off `table.select()` yields the implicit join without a `select()` import.
- The PostToolUse hook auto-formats edited files — and auto-removes imports that are momentarily unused, *including on edits to a different region of the file*: imports added in one Edit and used in a later Edit get stripped in between. Add usages with (or before) their imports, and fix imports last when extending an existing module.
