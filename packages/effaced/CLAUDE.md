...
| `retention/` | `RetentionSweeper(data_map, graph, metadata, audit_sink)`, `RetentionReport`/`RetentionReportEntry` | report-only sweep (ADR 0012): SELECTTs only, never mutates; sweepable iff policy has `duration` + `anchor`, everything else counted indeterminate (never guessed); eligibility ignores erasure strategy; one `RETENTION_EXPIRED` event per subject with expired rows (names + counts, never values/timestamps); the planner stays time-free — a delete mode is a future MAJOR ADR |
