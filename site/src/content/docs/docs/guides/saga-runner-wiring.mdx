...
description: Drive SagaRunner from the worker, cron job, or background thread your application already has.
sidebar:
  order: 1
---
...
The outbox carries Art. 16 fan-out too: rectify entries
...
```sql
ALTER TABLE effaced_outbox
  ADD COLUMN operation VARCHAR(32) NOT NULL DEFAULT 'erase';
ALTER TABLE effaced_outbox ADD COLUMN payload JSONB;
```
...
```sql
SELECT subject_id, resolver, operation, last_error, attempts
