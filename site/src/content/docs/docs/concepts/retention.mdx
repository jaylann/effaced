...
Retention also runs the other way: some external systems hold PII that
...
[`RetentionOnlyResolver`](../resolvers/) *schedules* erasure instead of
performing it, and the [saga](../saga/) records that schedule as what it
...
the trail exists to prevent (ADR 0018).
...
The report-only sweep above is untouched: it evaluates *your database's*
declared windows and never consults resolvers. The saga is the
enforcement clock for external horizons.
